...
import top.maary.emojiface.ui.components.SaveButtonCompact
import top.maary.emojiface.ui.components.SettingsButton
import top.maary.emojiface.ui.components.ShareButton
import top.maary.emojiface.ui.components.ShareButtonCompact
import top.maary.emojiface.ui.components.SliderWithCaption

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun EditScreenLarge(emojiViewModel: EmojiViewModel = viewModel(), isMedium: Boolean) {
    ...
    Scaffold(
        ...
        NavigationSuiteScaffold(
            navigationSuiteItems = {
                ...
                if (currentImage != null) {
                    item(
                        ...
                        label = {  },
                        selected = false,
                        onClick = { emojiViewModel.clearImage() }
                    )
                }
            },
            ...
            Row(
                ...
                Card(modifier = Modifier.weight(1f).fillMaxHeight().padding(end = 8.dp),
                    colors = CardDefaults.cardColors(
                        containerColor = MaterialTheme.colorScheme.surfaceContainerLow
                    )) {
                    Column(modifier = Modifier.fillMaxHeight(), verticalArrangement = Arrangement.SpaceBetween) {
                        LazyVerticalGrid (columns = GridCells.Adaptive(minSize = 76.dp),  modifier = Modifier.weight(1f)) {
                        ...
                        Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
                            Row(
                                ...
                                if (resultBitmap != null) {
                                    if (isMedium) {
                                        ShareButtonCompact(backgroundColor = MaterialTheme.colorScheme.secondary) {
                                            // 获取当前显示的 Bitmap
                                            val bitmap =
                                                resultBitmap?.asImageBitmap()?.asAndroidBitmap()
                                            bitmap?.let { emojiViewModel.shareImage(it) }
                                        }
                                        SaveButtonCompact(backgroundColor = MaterialTheme.colorScheme.secondary) {
                                            val bitmap =
                                                resultBitmap?.asImageBitmap()?.asAndroidBitmap()
